...
impl<'a, A: ToSocketAddrs> JsonForwardable for Fluent<'a, A> {
    ...
    fn post<T>(self, record: T, max_retry: u64, multiplier: f64) -> Result<(), FluentError>
    where
        T: Serialize + Debug + Clone,
    {
        let time = time::now();
        self.post_with_time(record, time, max_retry, multiplier)
    }

    /// Post record into Fluentd. With time version.
    fn post_with_time<T>(self, record: T, time: time::Tm, max_retry: u64, multiplier: f64) -> Result<(), FluentError>
    where
        T: Serialize + Debug + Clone,
    {
